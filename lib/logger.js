...
Logger.prototype.logMessage = function (level, msg) {
  ...
  if(level < Logger.LOG_INFO) {
    str = this.levelNames[level] + "\t" + this.hostname + "\t" + process.pid + "\t" + this.extractCalledFromStack() + "\t" + msg;
  } else {
    str = this.levelNames[level] + "\t" + this.hostname + "\t" + process.pid + "\t\t" + msg;
  }
  ...
  if (this.scribe) {
    ...
    if(level < Logger.LOG_INFO) {
      ...
      process.stdout.write(this.levelNames[level] + "\t" + process.pid + "\t\t" + msg + "\n");
    }
  }
};
